...
printfn "\tAdding drone arg : %A" ((0, 0), (3, 0), 300)
aDrone.AddDrone (Drone ((0, 0), (3, 0), 300))
...
for i = 0 to (List.length aDrone.Drones)-1 do
    printfn "\tDrone %A at dest : %b" i (aDrone.Drones.[i].IsCrashed)
// printf "Drone at dest : \n"
// for i = 0 to (List.length aDrone.Drones)-1 do
printfn "\t %A" (List.length (aDrone.WillCollide aDrone.Drones 1))
printf "Drone dist : \n"
aDrone.WillCollide aDrone.Drones 1
